...
```bash
emit-and-wait$ sam logs -n StockCheckerFunction --stack-name emit-and-wait --tail
```
...
```bash
emit-and-wait$ cd functions/stock-checker
...
```bash
aws cloudformation delete-stack --stack-name emit-and-wait
```

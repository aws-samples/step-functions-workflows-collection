...
class LambdaOrchestrationCdkStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        for l in lambdaDefList:
            ...
            handler_code = ""
            with open(f"lambda_orchestration_cdk/src/{l['srcName']}", encoding="utf8") as fp:
                handler_code = fp.read()

            # Create the current lambda function
            t = _lambda.Function(self, l['fnName'], code=_lambda.InlineCode(handler_code),
            ...
            _logs.LogGroup(self, f"{l['fnName']}Log",
        ...
        # privileges
        self.buySellRole = iam_.Role(self, "BuySellRole",
        ...
        self.buySellFunction = _lambda.Function(self, "BuySellFunction",
                                                handler="index.handler",
                                                runtime=_lambda.Runtime.PYTHON_3_9,
        ...
        _logs.LogGroup(self, "BuySellFnLog",
                       removal_policy=RemovalPolicy.DESTROY,
                       retention=_logs.RetentionDays.ONE_DAY,
        ...
        self.queue = sqs_.Queue(self, "RequestApprovalQueue",
        ...
        self.state_machine_role = iam_.Role(self, "StateMachineExecRole",
                                            assumed_by=iam_.ServicePrincipal(
        ...
        self.state_machine_role.attach_inline_policy(iam_.Policy(self, "lambda-policy",
        ...
        self.topic = _sns.Topic(self, "AlertTopic",
        ...
        self.sfn_def = ""
        with open("lambda_orchestration_cdk/statemachine/statemachine.asl.json", encoding="utf8") as fp:
            self.sfn_def = fp.read()

        self.state_machine = _sfn.CfnStateMachine(self, "SampleStateMachine",
                                                  role_arn=self.state_machine_role.role_arn,
                                                  definition_string=self.sfn_def,
                                                  definition_substitutions={"checkPriceFnArn": self.lambdaList["check-price"].function_arn,
        ...
        self.buySellFunction.add_to_role_policy(iam_.PolicyStatement(
...
# Outputs
        outputs_(self, "StateMachineName",
                 value=self.state_machine.attr_name)
        outputs_(self, "StateMachineArn",
